...
namespace BookingHutech.Controllers.Api
{
    public class AccountController : ApiController
    {
        ...
        /// <returns>ApiResponse</returns>
        [HttpPut]
        public ApiResponse ChangePassword([FromBody] ManagerUpdateAccountRequestModel request)
        {
            ManagerAccountServices managerAccountServices = new ManagerAccountServices();
            try
            {
                ...
                if (Permissions.CheckAPIRequest(Request.Headers.GetValues(ApiHeaderKey.BHAPIWebCall.ToString()).First()) == (int)ApiRequestType.Web)
                {
                    try
                    {
                        ...
                        // gọi hàm kiểm tra mật khẩu. 
                        if (DataEntity.CheckPassWord(request.Password))
                        {
                            try
                            {
                                request.Password = EncodePassword.CreateSHA256(request.Password);
                                managerAccountServices.ChangePasswordServices(request);
                                ManagerGetDetailAccountByAccountIDRequestModel reqDetailAcc = new ManagerGetDetailAccountByAccountIDRequestModel();
                                reqDetailAcc.Account_ID = request.Account_ID;  
                                var result = managerAccountServices.ManagerGetDetailAccountByAccountIDServices(reqDetailAcc); 
                                return ApiResponse.Success(result);
                            }
                            catch (Exception )
                            { 
                                // trùng mật khẩu
                                return ApiResponse.PasswordAlreadyExist(); 
                            }
                        }
                        else
                        {
